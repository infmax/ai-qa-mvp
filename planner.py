...
def _shrink_inventory(inv: List[Dict[str, Any]], max_items: int = 120) -> List[Dict[str, Any]]:
    if not inv:
        return []
    out = []
    for e in inv[:max_items]:
        out.append({
            ...
            "role": e.get("role"),
            "testid": e.get("testid"),
            "placeholder": (e.get("placeholder") or "")[:80] if e.get("placeholder") else None,
...
def _build_user_prompt(
    step_id: str, step_title: str,
    dane: str, action: str, result: str,
    url: str, title: str, body_html: str, dom_inventory: List[Dict[str, Any]],
...
<<<HTML_END>>>
    ...
    Напомню: разрешённые action/selector.type перечислены в системных правилах.
    """)
...
def _pydantic_load_or_repair(raw: str, fallback_title: str, step_id: str, hints: Dict[str, Any] | None) -> StepPlan:
    try:
        data = json.loads(raw)
    except Exception:
        return StepPlan(stepId=step_id, title=fallback_title, instructions=[], expects=[], hintsFromUser=hints)
    try:
...
def plan_step_llm(
    step_id: str, step_title: str,
    dane: str, action: str, result: str,
    url: str, title: str, body_html: str, dom_inventory: list,
    hints: Dict[str, Any] | None = None
) -> StepPlan:
    user_prompt = _build_user_prompt(
        step_id, step_title, dane, action, result,
        url, title, body_html, dom_inventory, hints
    )
    ...
    if hints:
        ...
        if isinstance(so, dict):
            ...
            sel = so.get("selector")
            if isinstance(idx, int) and 0 <= idx < len(plan.instructions) and isinstance(sel, dict):
                try:
                    if plan.instructions[idx].target:
                        plan.instructions[idx].target.selector = Selector(**sel)
                    else:
                        plan.instructions[idx].target = Target(selector=Selector(**sel), alternatives=[])
                except Exception:
                    pass
